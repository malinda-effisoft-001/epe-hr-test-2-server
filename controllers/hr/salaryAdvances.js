...
exports.search = async (req, rpp, page, callBack) => {
  ...
  if(rpp===0){
    ...
    try{
      const { count, rows } = await salaryAdvance.findAndCountAll({
        attributes: ['id', 'u_date', 'u_time', 'user_id', 'estate_id', 'division_id', 'employee_id', 'amount', 'status'],
        include: [
          {
            model: db.user,
...
exports.issue = (req, callBack) => {
  ...
  .then(data=>{
    var result = {id: data.id, u_date: u_date, u_time: u_time, status: 'finalized'};
    ...
    .then(data1=>{
      ...
      db.shopAccount.create({
        ...
        division_id: req.division_id,
        employee_id: req.employee_id,
        tra_description: 'Salary Advance',
        tra_ref: data.id,
        tra_item_ref: -1,
        tra_type: 'CREDIT',
        credit: credit,
