...
exports.findOne = (req, callBack) => {
    job.findOne({
        attributes: ['id', 'code', 'description', 'shop_pay_type', 'shop_pay_amount', 'color', 'status'],
        where: {
          id: req.id
        }
    })
...
exports.findActive = (req, callBack) => {
    ...
    where.status = "active";
    job.findAndCountAll({
        attributes: ['id', 'code', 'description', 'color', 'shop_pay_type', 'shop_pay_amount', 'status'],
        where: where
    })
    .then(data=>{
        callBack({error:false, data:data, errorMessage:""});
  ...
  .then(data=>{
    if(!data){
      job.create({
        ...
        shop_pay_type: req.body.shopPayType,
        shop_pay_amount: req.body.shopPayAmount,
